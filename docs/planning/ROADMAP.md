...
```mermaid
flowchart TD
    Phase13["Phase 13: 极美排版与纸质质感版 (v1.8.0 / v1.8.1)"] --> P13_1["避头尾禁则与悬挂 / 抽屉等宽排版 / 双页直选"]
    
    Phase14["Phase 14: 微软 Edge-TTS 拟人神经语音与听书生态"] --> P14_1["Edge-TTS 晓晓/云希等超自然神经语音"]
    Phase14 --> P14_2["Legado 3.0 有声书源音频流边下边播"]
    Phase14 --> P14_3["Windows SMTC 全局媒体控制与悬浮播放面板"]
```

- **阶段十四候选方向规划**：
  1. **方向一（推荐）：微软 Edge-TTS 超自然神经语音与有声书生态**
     - 集成 Microsoft Edge-TTS 云端免费神经语音（晓晓、云希、云扬等高质量声线）；
     - 支持 Legado 3.0 在线音频书源播放与进度断点记忆；
     - 接入 Windows 系统级媒体控制（SMTC），支持耳机/键盘多媒体键与锁屏常驻控制。
  2. **方向二：全书离线多格式打包导出**
     - 支持将已缓存或本地书籍整本导出为标准 TXT、EPUB 或排版清晰的 PDF 文档。
  3. **方向三：AI 智能阅读助手**
     - 接入大模型 API 或本地 Ollama，支持长篇章节内容提炼摘要与登场人物脉络图谱。
