...
### 阅读器排版与交互体验深度优化版 (v1.8.1) — 已交付
- **M13.7 设置抽屉自适应等宽网格排版**：
  - 彻底修复 360dp 设置抽屉 FilterChip 挤压变形、文字换行折叠为单列竖排问题；
  - 统一采用 `Modifier.weight(1f)` 均分 1/3 宽度并居中，文字精炼为 `25:50:25`、`33:34:33`、`20:60:20` 及 `紧凑 (16)`、`适中 (32)`、`宽裕 (64)`。
- **M13.8 连续垂直滚动模式下段落间距毫秒级响应**：
  - 解决单一 Text 组件无法响应段落间距问题，重构为段落级流式布局渲染，段间距滑块实时响应。
- **M13.9 左右双页对开排版进入设置抽屉**：
  - 设置抽屉直选「左右双页对开」对称排版选项，并与顶部工具栏双向同步与持久化。
- **M13.10 滚轮与左右点击交互明确化**：
  - 滚轮专用于章节内平滑滚动，左右区域点击分别直达上一章与下一章，并自动兼容旧配置。
- **M13.11 默认禁用首字下沉 (Drop Caps)**：
  - 首字下沉默认置为关闭，保障阅读版面纯净与一致性。

---

## 3. 未来路线规划与交付阶段 (Phase 14 ~ Phase 16)

```mermaid
flowchart TD
    Phase13["Phase 13: 极美排版与排版优化 (v1.8.0 / v1.8.1)"] --> Phase14["Phase 14: 书源生态中台与一键智能换源 (v1.9.0)"]
    Phase14 --> Phase15["Phase 15: 全书离线多格式打包导出引擎 (v1.9.5)"]
    Phase15 --> Phase16["Phase 16: AI 智能阅读辅助生态 (v2.0.0)"]

    subgraph Phase14_Details["Phase 14 核心模块"]
        P14_1["书源并发健康测速与失效诊断引擎 (可用/超时/失效/空内容)"]
        P14_2["书源中台管理 (批量启用/禁用/删除、分组管理与置顶)"]
        P14_3["复杂规则解析与 Rhino JS 沙箱兼容性深度增强"]
        P14_4["搜索结果同名书籍聚合与阅读器内「🔄 一键无感换源」"]
    end

    subgraph Phase15_Details["Phase 15 核心模块"]
        P15_1["单文件精排规范 TXT 导出 (标题标准化/空行清理/自动缩进)"]
        P15_2["标准电子书 EPUB 打包导出 (OCF标准/元数据/封面/目录导航树)"]
    end

    subgraph Phase16_Details["Phase 16 核心模块"]
        P16_1["通用大模型接口兼容 (OpenAI规范 / DeepSeek / 本地Ollama)"]
        P16_2["阅读器侧边轻量「AI 助手」抽屉 (章节速读摘要/词句白话释义/人物图谱)"]
    end

    Phase14 --- Phase14_Details
    Phase15 --- Phase15_Details
    Phase16 --- Phase16_Details
```

### 阶段十四：书源生态中台、健康测速诊断与一键智能换源 (Phase 14 — v1.9.0) — 已交付
- **M14.1 书源健康度诊断与并发测速引擎 (`BookSourceDiagnosticEngine`)**：
  - 基于协程并发池（`Semaphore(8)`）对用户已导入的书源执行网络连通性、搜索响应延迟与章节正文有效性批量检测；
  - 自动归类标记【正常 (🟢 绿色/低延迟)】、【超时 (🟡 黄色/网络延迟高)】、【失败 (🔴 红色/规则报错/无法连接)】、【无数据 (⚠️ 橙色/解析结果为空)】、【未测速 (⚪ 灰色)】；
  - 书源管理界面支持一键测速、一键禁用全部失效源、一键清理删除失效源（带二次确认）。
- **M14.2 书源中台分组与过滤管理 (`SourcesView`)**：
  - 支持关键字实时搜索过滤（匹配书源名与网址）；
  - 支持书源分组（提取全部标签并动态平滑横向滚动展示，支持切换全部、未分组或各自定义标签）。
- **M14.3 复杂规则与 Rhino JS 兼容深度增强 (`JsEngine` & `BookSourceEngine`)**：
  - 扩充 JS 沙箱内置宿主函数：支持两参数 `java.getString(json, rule)` 规则提取、`java.timeFormat` 时间戳多载格式化、`java.md5Encode`、`java.encodeURI`/`java.decodeURI`；
  - 完善防爬请求头支持：`parseHeaderMap` 解析书源 `header` 字段，注入自定义 `User-Agent`、`Referer`、`Cookie` 到网络请求。
- **M14.4 搜索结果同名书籍聚合与阅读器内「🔄 一键无感换源」**：
  - 搜索结果将同名同作者书籍智能合并为单张卡片，展示【N 个可用书源】并提供候选源选择弹窗；
  - 阅读器顶部工具栏新增「🔄 换源」快捷入口，点击弹出抽屉/对话框，列出该书所有来源、最新章节名、当前使用状态；
  - 点击目标来源即刻完成无缝切源，并自动按章节名和序号对齐到当前阅读进度，彻底解决正文缺章乱码断更困扰。
- **M14.5 单元测试套件扩充与质量基准守护**：
...
- **M16.1 通用大模型设置与协议适配 (`AiAssistantEngine`)**：
  - 适配 OpenAI 兼容标准接口协议，支持配置 Base URL、API Key 与 Model 名称（兼容 DeepSeek-V3、通义千问、Kimi、OpenAI 等，支持本地部署的 Ollama 0 成本运行）；
  - 严格遵守安全准则，API Key 仅安全保存在本地 SQLite 数据库中，不上传至任何第三方。
- **M16.2 阅读器侧边「AI 助手」轻量交互抽屉 (`AiAssistantDrawer`)**：
  - 阅读器右上角提供「AI 助手」轻量抽屉，随时一键滑出，不破坏正文沉浸式阅读空间；
  - **章节速读摘要**：一键提炼当前章节核心脉络、关键转折与剧情速览；
  - **划选深度释义**：选中文本时可直接发送至 AI 助手，快速解析文言词句、生僻词汇、历史典故与角色前因后果；
  - **角色脉络谱系**：智能梳理全书登场人物关系网。
