...
- **沉浸式阅读与仿真滑动翻页**：
  ...
  - **经典仿真横向滑动翻页**：像素级 CJK/ASCII 字符宽度加权虚拟分页算法，配合 Compose 平滑横向左右推拉动效，页脚实时显示 `第 X / Y 页 · 本章共 Z 字`。
  - **五维精细排版控制台**：字体大小、行间距 (1.2x ~ 2.5x)、段间距 (0 ~ 32dp)、首行全角双字符缩进开关、页面边距 (16/32/64dp)。
  - **F11 全屏沉浸模式**：一键进入无干扰全屏沉浸阅读。
  - **高度自由定制**：在阅读设置中提供可视化三栏预览卡片，支持自由调节分区比例（25:50:25 / 33:34:33 / 20:60:20）与动作映射。
  - **单页 / 双页 / 条漫模式切换**：大屏双栏对开排版（含拟真书脊阴影），体验实体纸质书对开质感；支持图文瀑布流条漫模式。
  - **侧边非模态实时设置抽屉 (SettingsDrawer)**：360dp 右滑沉浸抽屉，调整字号、行距、边距及翻页模式时正文实时响应重排，告别遮挡式弹窗，所见即所得。
  - **8 套经典护眼与沉浸主题矩阵**：明亮白、复古羊皮纸、经典豆沙绿、黄昏落日、深海蔚蓝、黑白墨水屏、OLED纯黑、淡雅薄荷绿，并支持 Hex 自由调色。
  - **章节内原生高亮检索 (`Ctrl+F`)**：浮动搜索栏、琥珀色 Skia GPU 文字级高亮、匹配总数统计、Enter/Shift+Enter 双向跳转并智能联动双页切页与平滑滚动。
  - **划线摘录与剪贴板智能书签**：全局接入 Windows 原生光标文本选中复制 (`SelectionContainer`)，一键书签弹窗自动捕获当前章节选段作为引用笔记。
  - **绿色便携存储与百兆流式秒开 (`RandomAccessFile`)**：本地书库优先存放在软件解压根目录 `./local_books/`，拷走即用；基于单遍字节偏移流式扫描与精准 Seek 读取，彻底杜绝碎片小文件，百兆小说 200ms 秒开入库。
  - **Windows 全局拖拽入库与分章自愈面板**：支持将 `.txt`/`.epub` 文件拖拽入窗口，全屏浮现沉浸靶心动效；目录抽屉内置「分章微调」自愈面板，提供 4 套经典预设与自由正则实时预览重整。
  - **选中文本浮动拟物胶囊与 Skia 双层渲染**：选中文本即弹出悬浮药丸工具栏，支持 🟡/🟢/🟣 三色高亮、〰️ 下划线、✏️ 记录想法、📋 复制与 🔊 朗读；Skia 原生底图层划线与顶图层搜索高亮无冲突渲染。
  - **知识管理与结构化 Markdown 导出**：目录抽屉「划线与笔记」三 Tab 独立面板，支持单条定位与删除；一键复制结构化 Markdown 笔记至剪贴板，支持原生另存为 `.md` 文件，直通 Obsidian/Notion 等知识库。
  - **中文严格避头尾禁则与标点智能微悬挂 (`Kinsoku Shori`)**：遵守中文排版学禁则，后置标点（逗号、句号、闭括号等）绝不单独落于行首，前置标点（开引号、书名号等）绝不孤立在行尾；支持行末 0.1 字宽标点智能悬挂与安全回退折行，版心端正优雅。
  - **首段首字艺术大字下沉 (`Drop Caps`)**：章节首段首字以 28sp 特粗体（ExtraBold）艺术化呈现，赋予长篇典籍如古籍孤本般沉稳典雅的历史厚重美感。
  - **卷轴风卷首艺术装饰分割线**：在章节标题正下方呈现 `❖ ─── ✦ ─── ❖` 古典图腾，增添翻章仪式感与古典意境。
  - **桌面级无边框图文插图原生灯箱 (`Image Lightbox`)**：点击插图或漫画条漫即刻拉起全屏暗化原生灯箱画廊，支持鼠标滚轮无级缩放（0.5x ~ 5.0x）、高倍率下鼠标拖拽全向平移原图、双击 100% 极速复位。
  - **Skia 原生温润纸质微噪点底衬 (`PaperTextureCanvas`)**：基于 Skia 实时渲染微米级微噪点网格算法，带来如墨水屏般温润柔和的纸质触感，彻底消除屏幕发白刺眼感；0 额外图片体积，2% ~ 20% 浓度自由调节与 SQLite 持久化。

### ⚡ 强大的书源规则引擎 (Legado 3.0 兼容)
- **多维度智能加权搜索排序引擎**：彻底解决搜书偏差大的问题，书名精准匹配置顶（+100,000 分）、标点剥离归一化、作者匹配与元数据加权，让正品原著稳居首位。
- **离线批量缓存引擎与存储管理**：支持后 50 章、后 100 章、至最新章节、全本书籍高速离线批量缓存，3 协程并发 + 100ms 动态防封时延，本地秒开零延迟；系统原生目录选择器，支持自由迁移缓存至大容量非系统盘；单本与全库体积统计，一键安全清理。
- **全局非阻塞悬浮胶囊指示器**：右下角常驻显示下载进度、当前/总章节与书名，支持后台无感下载与一键中止；书架卡片与阅读器均有一键缓存入口。
...
# 打包产物输出路径: build\compose\jars\LegadoDesktop-windows-x64-1.8.0.jar
```
...
| **🗺 开发规划路线图** | [docs/planning/ROADMAP.md](docs/planning/ROADMAP.md) | Phase 1~13 (v1.8.0 极美排版与纸质质感版) 全演进阶段、关键完成标准与未来规划 |
| **🧪 测试与性能基准** | [docs/testing/TESTING_GUIDE.md](docs/testing/TESTING_GUIDE.md) | 18大单元测试套件（69项全量自动化测试矩阵 100% 通过）、启动耗时与内存量化指标 |
| **🛡 安全与合规审计** | [docs/audit/AUDIT_REPORT.md](docs/audit/AUDIT_REPORT.md) | Rhino JS 沙箱防逃逸、便携目录安全探测、流式切片防越界、排版防死循环、图库手势安全 |
| **💡 重大技术突破** | [docs/breakthroughs/CORE_BREAKTHROUGHS.md](docs/breakthroughs/CORE_BREAKTHROUGHS.md) | 规则引擎、发现榜单、字体排版、双页对开、设置抽屉、深度批注、避头尾、首字下沉等24大突破剖析 |
